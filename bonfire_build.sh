...
function podman_build {
    ...
    podman build --arch=amd64 --manifest "${IMAGE}:${IMAGE_TAG}" -f $APP_ROOT/$DOCKERFILE $APP_ROOT
    podman build --arch=arm64 --manifest "${IMAGE}:${IMAGE_TAG}" -f $APP_ROOT/$DOCKERFILE $APP_ROOT
